# ==============================================================================
#  helpers/gpu-driver-clean.ps1  -  Native Safe Mode GPU Driver Removal
# ==============================================================================
...
function Get-GpuVendorApplicationCleanupTargets {
    ...
    $relativeTargets = switch ($GpuVendor) {
        ...
        # AMD-wide roots can also contain chipset and Ryzen Master software.
        # The alpha does not select those roots for recursive cleanup.
        'AMD' { @() }
        default { @() }
    }
...
function Remove-GpuDriverClean {
    ...
    Write-Info "This repository uses its own Windows-native removal workflow."
    Write-Info "Its scope and behavior are not claimed to match DDU."

    if ($SCRIPT:DryRun) {
        Write-ConsoleLine "  [DRY-RUN] Would remove matching $GpuVendor display-driver packages and selected rebuildable residue" -ForegroundColor Magenta
        ...
        Write-ConsoleLine "  [DRY-RUN]   3. After verified removal, apply the vendor-specific residue allowlist" -ForegroundColor Magenta
    ...
    try {
        ...
        foreach ($svc in $services) {
            try {
                ...
                $serviceStepTitle = "GPU Driver Clean ($GpuVendor)"
                $capture = Backup-ServiceState -ServiceName $svc.Name -StepTitle $serviceStepTitle -PassThru
                ...
                if (@($durableBackup.entries | Where-Object {
                    $_.type -eq 'service' -and $_.step -eq $serviceStepTitle -and $_.name -eq $svc.Name
                }).Count -eq 0) {
    ...
    if ($GpuVendor -eq "NVIDIA") {
        ...
        Write-Info "AMD-wide application cleanup is disabled to preserve chipset and Ryzen Master software."
    }
    ...
    if ($GpuVendor -eq "NVIDIA") {
        ...
        Write-Info "AMD-wide registry cleanup is disabled to preserve non-display AMD software state."
    }
