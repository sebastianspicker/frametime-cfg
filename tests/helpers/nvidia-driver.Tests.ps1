...
Describe "Get-LatestNvidiaDriver" {
    ...
    Context "current NVIDIA result-page response" {
        ...
        It "resolves the linked result record through NVIDIA's details service" {
            $script:DriverDetailsJson = @{
                driverDetails = @{
                    IDS = @(@{
                        downloadInfo = @{
                            Success = "1"
                            ID = "274187"
                            DownloadURL = "https://us.download.nvidia.com/Windows/610.74/driver.exe"
                            Version = "610.74"
                        }
                    })
                }
            ...
            Mock Invoke-WebRequest {
                if ([string]$Uri -match 'processFind\.aspx') {
                    return [PSCustomObject]@{
                        Content = "<a href='//www.nvidia.com/download/driverResults.aspx/274187/en-us'>Driver</a>"
                    }
                }
                if ([string]$Uri -match 'getDownloadDetails') {
                    return [PSCustomObject]@{ Content = $script:DriverDetailsJson }
                }
                throw "Unexpected NVIDIA URL: $Uri"
            }

            $result = Get-LatestNvidiaDriver

            $result.ManualDownload | Should -BeFalse
            $result.Version | Should -Be "610.74"
            $result.Url | Should -Be "https://us.download.nvidia.com/Windows/610.74/driver.exe"
            Should -Invoke Invoke-WebRequest -Exactly 2
        }

        It "rejects details metadata for a different result record" {
            $script:DriverDetailsJson = @{
                driverDetails = @{
                    IDS = @(@{
                        downloadInfo = @{
            ...
            $result.ManualDownload | Should -BeTrue
            $result.Url | Should -Be "https://www.nvidia.com/en-us/drivers/"
        }

        It "applies the NVIDIA host allowlist to two-stage metadata" {
            $script:DriverDetailsJson = @{
                driverDetails = @{
                    IDS = @(@{
                        downloadInfo = @{
...
Describe "Install-NvidiaDriverClean" {
    ...
    It "executes exactly one contained NVIDIA-signed setup.exe and accepts exit code zero" {
        ...
        Mock Get-NvidiaDisplayDriverSnapshot {
            $script:SnapshotCalls++
            if ($script:SnapshotCalls -eq 1) { return $null }
            [PSCustomObject]@{
                Identity = "PNP:PCI\VEN_10DE&DEV_2684"
                Name = "NVIDIA GeForce RTX 4090"
                Version = "32.0.15.7242"
        ...
        Should -Invoke Get-AuthenticodeSignature -Exactly 1 -ParameterFilter { $FilePath -eq $setupPath }
        Should -Invoke Get-NvidiaDisplayDriverSnapshot -Exactly 2
    }

    It "rejects an accepted setup exit code when no NVIDIA display driver is present afterward" {
        $SCRIPT:DryRun = $false
        $driver = Join-Path $SCRIPT:TestTempRoot "unverified-success-driver.exe"
        $extractRoot = Join-Path $SCRIPT:TestTempRoot "unverified-success-extraction"
        $setupPath = Join-Path $extractRoot "setup.exe"
        $securedDriver = Join-Path $extractRoot "nvidia-driver-package.exe"
        New-Item -ItemType Directory -Path $extractRoot -Force | Out-Null
        Set-Content -Path $driver -Value "fake driver" -Encoding UTF8
        ...
        $result | Should -BeFalse
        Should -Invoke Get-NvidiaDisplayDriverSnapshot -Exactly 2
        Should -Invoke Write-Err -ParameterFilter { $t -match 'no NVIDIA display driver was found afterward' }
...
Describe "Apply-NvidiaPostInstallTweaks" {
    ...
    It "disables NVIDIA telemetry registry entries" {
        ...
        $result = Apply-NvidiaPostInstallTweaks
    ...
    It "reports a partial result when a registry change fails" {
        Mock Set-RegistryValue {
            if ($name -eq "OverlayTestMode") {
                return [PSCustomObject]@{ Status = "Failed"; Applied = $false }
            }
            [PSCustomObject]@{ Status = "Success"; Applied = $true }
        }
        Mock Write-Step {}
        ...
        Should -Invoke Write-Warn -ParameterFilter { $t -match "MPO registry value was not applied" }
    }
}
