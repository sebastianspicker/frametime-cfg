...
Describe "Remove-GpuDriverClean" {
    ...
    Context "truthful cleanup outcomes" {
        ...
        It "does not count a failed vendor application directory deletion as software removed" {
            $script:cimCalls = 0
            Mock Get-CimInstance {
                $script:cimCalls++
                if ($script:cimCalls -eq 1) {
            ...
            Mock Get-GpuVendorApplicationCleanupTargets {
                [PSCustomObject]@{ Root = 'C:\Program Files'; Path = 'C:\Program Files\AMD' }
            }
            Mock Test-Path {
                param($LiteralPath, $Path)
                $LiteralPath -eq 'C:\Program Files\AMD'
            }
            Mock Test-GpuCacheCleanupTarget { $true }
            Mock Remove-Item { throw 'locked' } -ParameterFilter { $LiteralPath -eq 'C:\Program Files\AMD' }
            $result = Remove-GpuDriverClean -GpuVendor AMD -PassThru
        ...
        It "marks vendor registry deletion failure as partial instead of printing success" {
            $script:cimCalls = 0
            Mock Get-CimInstance {
                $script:cimCalls++
                if ($script:cimCalls -eq 1) {
