...
pub(crate) fn manifest_for(
    ...
    let expected = frametime_core::runtime::RUNTIME_PAYLOAD_PATHS
        .into_iter()
        .collect::<std::collections::BTreeSet<_>>();
    if actual != expected {
    ...
    let executable = files
        .get("frametime.exe")
        .cloned()
        .ok_or("compiled payload does not include frametime.exe")?;
    Ok(frametime_core::runtime::RuntimeManifest {
        ...
        executable: frametime_core::runtime::RuntimeExecutableRecord {
