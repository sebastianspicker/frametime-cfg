...
document.addEventListener("keydown",event=>{if(!event.ctrlKey||event.altKey||event.metaKey)return;const index=Number(event.key)-1;if(index>=0&&index<nav.length){event.preventDefault();showPanel(nav[index].dataset.panel)}});
